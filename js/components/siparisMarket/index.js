...
class MarketMain extends Component {
    ...
    loadItems() {
        ...
        if (this.state.data.products.length === 0 && !this.props.loaderVisible) {
            productList.push(
                <View style={[accountAndMarket.accountNoHistoryCard, accountAndMarket.cardView]} key={'noMarketItem'} >
                    <Image source={require('../../../images/nothing-here.png')} style={accountAndMarket.accountNothingImage} />
                    <Text style={accountAndMarket.marketNoHistoryText} >Mağazada ürün bulunmamaktadır.</Text>
                </View>
            );
        }
        else {
            this.state.data.products.forEach(function (tmpProduct) {
                productList.push(
                    <View style={accountAndMarket.cardView} key={tmpProduct.id} >
                        ...
                        <Grid style={accountAndMarket.upperGrid} >
                            ...
                            <Col style={{paddingLeft: 10}}>
                                <TouchableOpacity onPress={() => this._onProductTextClick(tmpProduct)} >
                                    <Text style={accountAndMarket.cardItemHeader} numberOfLines={2} > {tmpProduct.title} </Text>
                                    <Text style={accountAndMarket.cardItemBody} numberOfLines={2} > {tmpProduct.description} </Text>
                                </TouchableOpacity>
                            </Col>
                        </Grid>
                            ...
                            <Grid style={{flex: 1, alignItems: 'center',borderBottomWidth: 1,borderBottomColor: 'rgba(236, 236, 236, 0.82)'}} key={tmpUrl + '_deeplink'}>
                                <Col style={{}}>
                                    <View style={accountAndMarket.profileInfo}>
                                        <TouchableOpacity onPress={() => this._onChangeMarket(tmpUrl)} >
                    ...
                    <Modal
                        ...
                        <View>
                            ...
                            <Grid style={{flex: 1, alignItems: 'center',borderBottomWidth: 1,borderBottomColor: 'rgba(236, 236, 236, 0.82)'}}>
                                <Col style={{}}>
                                    <View style={accountAndMarket.profileInfo}>
                                        <Thumbnail source={{ uri: 'data:image/png;base64,' + this.props.user.avatarImage }} style={accountAndMarket.profilePic} />
                                    </View>
                                </Col>
                                <Col style={{flex: 3, margin: 5}}>
                                    <Text style={accountAndMarket.cardItemHeader} >{this.state.accountBalance.integerPart.replace(/\B(?=(\d{3})+(?!\d))/g, ".")},{this.state.accountBalance.fractionalPart} Keklik</Text>
                                </Col>
                            </Grid>
